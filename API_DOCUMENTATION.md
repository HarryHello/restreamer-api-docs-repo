...
本系统设计为**高度自动化**，大部分功能无需前端调用 API 即可自动执行：

#### 🔄 自动执行的功能（无需前端调用）

| 功能 | 说明 |
|------|------|
| **字幕录制** | 当频道监控检测到直播开始时，自动启动字幕录制；直播结束时自动停止 |
| **直播状态监控** | 启动监控后，系统自动定期检测直播状态变化 |
| **OBS 自动化** | 检测到直播开始/结束时，自动切换 OBS 场景、启动/停止推流或录制 |
| **字幕翻译** | 根据配置自动进行字幕翻译（如启用） |

#### 📋 需要前端调用的功能

| 功能 | 说明 |
|------|------|
| **频道配置** | 设置频道 ID、平台等基本信息 |
| **启动/停止监控** | 手动控制监控的启停 |
| **实时状态查看** | 通过 SSE 获取频道状态更新 |
| **字幕文件管理** | 查看、下载、删除已录制的字幕文件 |
| **系统设置** | 配置各项功能和参数 |
| **OBS 手动控制** | 手动控制推流、录制、场景切换等 |

#### 💡 备用 API

部分 API 设计为**备用接口**，仅在特殊场景下需要手动调用：

- `/api/subtitles/process` - 手动处理字幕数据（通常自动执行）
- `/api/subtitles/session/start` - 手动启动字幕会话（通常自动执行）
- `/api/subtitles/session/end` - 手动结束字幕会话（通常自动执行）
- `/api/translation/translate/stream` - 手动翻译文本（通常自动执行）

**建议**：优先使用自动化功能，仅在需要精细控制或调试时使用备用 API。
